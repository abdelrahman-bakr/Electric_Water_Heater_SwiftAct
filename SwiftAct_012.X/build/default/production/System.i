...
void Task_ON_State(void){
                    ...
                    if(Temp_Measure_indication == 1){
                            ...
                            Actual_Temp = Temp_Sensor_ADC_Read();
                            Add_New_Temp(Temp_Measurments,Actual_Temp);
                            Actual_Temp_Avg = Calc_Temp_Avg(Temp_Measurments);
                            Temp_Measure_indication = 0 ;
                    }

                    Sev_Seg_Write_99(Actual_Temp);
                    Adjust_Temp();
...
void Task_ChangeTemp_State(void){
                      ...
                      if( ( Up_Pressed() == 1 ) && (Desired_Temp < 75) )
                      {
                          SettingMode_counter = 0 ;
                          Desired_Temp +=5 ;
                      }
                      if ( (Down_Pressed() == 1) && (Desired_Temp > 35) )
                      {
                          SettingMode_counter = 0 ;
                         Desired_Temp -=5 ;
                      }
...
void Adjust_Temp(void){
    ...
    if( (Desired_Temp < Actual_Temp_Avg + 1 ) && (Desired_Temp > Actual_Temp_Avg - 1 ) )
    {
         Fan_Stop();
         Heater_Stop() ;
         Heating_LED_Off();
    }
    else if( Desired_Temp > Actual_Temp_Avg + 1 ){
        Fan_Stop();
        Heater_Start();
        if(Toggle_indication == 0 ){
        Heating_LED_Toggle();
...
void Add_New_Temp(uint16_t* Array,uint16_t Newest_Value){
    ...
    for(uint8_t element = 10 -1 ; element > 0 ; element -- ){
